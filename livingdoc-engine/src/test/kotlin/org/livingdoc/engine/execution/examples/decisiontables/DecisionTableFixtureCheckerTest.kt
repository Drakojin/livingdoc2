...
internal class DecisionTableFixtureCheckerTest {
    ...
    @Test fun `before table methods must not have parameters`() {
        ...
        expectThat(errors).containsExactly("@BeforeTable method <static ${methodPrefix}BeforeTableWithParameter.beforeTable(java.lang.String)> has 1 parameter(s) - must not have any!")
    }

    @Test fun `before table methods must be static`() {
        val errors = executeCheck(MalformedFixtures.NonStaticBeforeTable::class)
        expectThat(errors).containsExactly("@BeforeTable method <${methodPrefix}NonStaticBeforeTable.beforeTable()> must be static!")
    }
    ...
    @Test fun `before row methods must not have parameters`() {
        ...
        expectThat(errors).containsExactly("@BeforeRow method <${methodPrefix}BeforeRowWithParameter.beforeRow(java.lang.String)> has 1 parameter(s) - must not have any!")
    }

    @Test fun `before row methods must not be static`() {
        val errors = executeCheck(MalformedFixtures.StaticBeforeRow::class)
        expectThat(errors).containsExactly("@BeforeRow method <static ${methodPrefix}StaticBeforeRow.beforeRow()> must not be static!")
    }

    @Test fun `two input fields cant share an alias`() {
        val errors = executeCheck(MalformedFixtures.InputFieldsWithSameAlias::class)
        expectThat(errors).containsExactly("Alias <input> is used multiple times!")
    }
    ...
    @Test fun `input methods must have exactly one parameter`() {
        ...
        expectThat(errors).containsExactly("@Input method <${methodPrefix}InputMethodWithoutParameter.input()> has 0 parameter(s) - must have exactly 1!")
    }

    @Test fun `input methods must not be static`() {
        val errors = executeCheck(MalformedFixtures.StaticInputMethod::class)
        expectThat(errors).containsExactly("@Input method <static ${methodPrefix}StaticInputMethod.input(java.lang.String)> must not be static!")
    }
    ...
    @Test fun `input methods and check methods cant share an alias`() {
        ...
        expectThat(errors).containsExactly("Alias <name> is used multiple times!")
    }
    ...
    @Test fun `before first check methods must not have parameters`() {
        ...
        expectThat(errors).containsExactly("@BeforeFirstCheck method <${methodPrefix}BeforeFirstCheckWithParameter.beforeFirstCheck(java.lang.String)> has 1 parameter(s) - must not have any!")
    }
    ...
    @Test fun `before first check methods must not be static`() {
        ...
        expectThat(errors).containsExactly("@BeforeFirstCheck method <static ${methodPrefix}StaticBeforeFirstCheck.beforeFirstCheck()> must not be static!")
    }

    @Test fun `check methods cant share an alias`() {
        val errors = executeCheck(MalformedFixtures.CheckMethodsWithSameAlias::class)
        expectThat(errors).containsExactly("Alias <check> is used multiple times!")
    }
    ...
    @Test fun `check methods must have exactly one parameter`() {
        ...
        expectThat(errors).containsExactly("@Check method <${methodPrefix}CheckMethodWithoutParameter.check()> has 0 parameter(s) - must have exactly 1!")
    }

    @Test fun `check methods must not be static`() {
        val errors = executeCheck(MalformedFixtures.StaticCheckMethod::class)
        expectThat(errors).containsExactly("@Check method <static ${methodPrefix}StaticCheckMethod.check(java.lang.String)> must not be static!")
    }

    @Test fun `after row methods must not have parameters`() {
        val errors = executeCheck(MalformedFixtures.AfterRowWithParameter::class)
        expectThat(errors).containsExactly("@AfterRow method <${methodPrefix}AfterRowWithParameter.afterRow(java.lang.String)> has 1 parameter(s) - must not have any!")
    }

    @Test fun `after row methods must not be static`() {
        val errors = executeCheck(MalformedFixtures.StaticAfterRow::class)
        expectThat(errors).containsExactly("@AfterRow method <static ${methodPrefix}StaticAfterRow.afterRow()> must not be static!")
    }
    ...
    @Test fun `after table methods must not have parameters`() {
        ...
        expectThat(errors).containsExactly("@AfterTable method <static ${methodPrefix}AfterTableWithParameter.afterTable(java.lang.String)> has 1 parameter(s) - must not have any!")
    }

    @Test fun `after table methods must not be static`() {
        val errors = executeCheck(MalformedFixtures.NonStaticAfterTable::class)
        expectThat(errors).containsExactly("@AfterTable method <${methodPrefix}NonStaticAfterTable.afterTable()> must be static!")
    }
